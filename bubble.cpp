...
void babelkowe_lista(LISTA** glowa, int n) {
	...
	LISTA* temp, * poprzedni_temp, * n_temp;
	for (int i = 0; i < n - 1; i++) {
		temp = *glowa;
		poprzedni_temp = NULL;
		for (int j = 0; j < n - 1 - i; j++) {
			n_temp = temp->nastepny;
			if (temp->liczba > n_temp->liczba) {
				if (poprzedni_temp != NULL) {
					poprzedni_temp->nastepny = n_temp;
				}
				else {
					*glowa = n_temp;
				}
				temp->nastepny = n_temp->nastepny;
				n_temp->nastepny = temp;
				poprzedni_temp = n_temp;
			}
			else {
				poprzedni_temp = temp;
				temp = temp->nastepny;
